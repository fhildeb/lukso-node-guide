...
After installing the Node Exporter, we will move on with the second exporter service for Prometheus: the JSON Exporter, as it's common practice to install the exporters before the main service, as [explained before](./02-node-exporter.md).
...
For the JSON exporter we will need the programming language Go, as the JSON exporter is a tool written in the Go programming language, to produce the binary executable.
...
#### Set JSON Explorer Permissionsets

Now we can change the owner of the JSON explorer service to the one that we created especially for this purpose:
...
After we copied the binary executable file into the system's program path and gave it the appropriate user rights, we can remove the original repository folder we've downloaded.
...
> Be cautious: When creating new rules or modifying existing ones, it's essential to follow the correct syntax and structure to ensure that the JSON Explorer functions properly and provides the desired level of security. Verify that you always use 2 spaces for each indentation and that the hyphen.
...
```sh
sudo chown json-exporter-worker:json-exporter-worker /etc/json_exporter/json_exporter.yml
```
...
- **ExecStart**: Specifies the command to run when the service starts. In this case, it's `/usr/local/bin/json_exporter` as program folder of the node explorer. We will also start it with our previously set up external data config file by passing it through the service using the `--config.file` flag.
...
### 7.3.6 Start the JSON Explorer Service
